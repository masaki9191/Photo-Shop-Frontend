...
function Form() {
    ...
    const handleSubmit = async() => {
        if(formValidate() === false)
            return;
        try {
            const data = await productAdd({ product : formValues });     
            setResponseMsg(data);
            setFormValues(formValueInit);
            setImages([]);       
            navigate("/");
        } catch (err) {
            console.log(err);
        ...
        <>
            <div className="sellPage">
                ...
                <div className="sellPage-registerBtn">
                    <p className="c-btn sellPage-subBtn" onClick={handleSubmit} >出品する</p>
                </div>
            </div>
        </>
    )
